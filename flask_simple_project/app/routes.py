...
def before_request():
    if current_user.is_authenticated:
        ...
        # The last step is to commit the database session, so that the change made above is written to the database.
        # If you are wondering why there is no db.session.add() before the commit, consider that when you reference
        # current_user, Flask-Login will invoke the user loader callback function, which will run a database query
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # The third case is in place to make the application more secure. An attacker could insert a URL to a
        # malicious site in the next argument, so the application only redirects when the URL is relative,
        # which ensures that the redirect stays within the same site as the application. To determine if the URL is
        # relative or absolute, I parse it with Werkzeug's url_parse() function and then check if the netloc
...
def user(username):
    ...
    prev_url = url_for('user', username=_user.username, page=posts.prev_num) \
        if posts.has_prev else None
    form = EmptyForm()
    return render_template('user.html', user=_user, posts=posts.items,
...
def follow(username):
    ...
    # so in that case I just redirect the application back to the home page.
    if form.validate_on_submit():
